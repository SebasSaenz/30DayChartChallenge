...
germany_semicircle <- parliament_data(
  election_data = germnany,
...
france_semicircle <- parliament_data(
  election_data = france,
  type = "semicircle", # Parliament type
  parl_rows = 4, # Number of rows of the parliament
...
spain_semicircle <- parliament_data(
  election_data = spain,
  type = "semicircle", # Parliament type
  parl_rows = 4, # Number of rows of the parliament
  party_seats = spain$n
...
(germany_plot + uk_plot) / (spain_plot + france_plot) +
  plot_annotation(
    title = "Eurovision",
    subtitle = "Number of countries that have <b style='color:#fd8d3c'>never awarded points</b> or have <b style='color:#49006a'>awarded points</b> to<br /> the Big Four in the Eurovision contest (1975-2022). Only viewers votes.",
    caption = caption,
    theme = theme(
